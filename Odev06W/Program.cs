...
public class Program : Form
{
    ...
    {
        if (potionCount > 0 && !heroes[heroIndex].IsUnconscious)
        {
    ...
    private void FightRound(object sender, EventArgs e)
    {
        ...
        foreach (var hero in heroes)
        {
            if (!hero.IsUnconscious && !dragon.IsUnconscious)
            {
    ...
    {
        bool allHeroesDead = heroes.All(h => h.IsUnconscious);
        if (dragon.IsUnconscious || allHeroesDead)
        {
            fightRoundButton.Enabled = false;
            string result = dragon.IsUnconscious ?
